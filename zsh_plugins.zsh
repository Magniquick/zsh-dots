...
# Load plugins.
ZSH_SMARTCACHE_DIR="${XDG_CACHE_HOME}/zsh"
source $ZPLUGDIR/zsh-smartcache/zsh-smartcache.plugin.zsh

# Generated using $(vivid generate catppuccin-mocha)
# https://github.com/sharkdp/vivid
if (($+commands[vivid])); then
	smartcache eval sh -c 'echo "export LS_COLORS=\"$(vivid generate catppuccin-mocha)\""'
else
...
if ! (($+commands[fzf])); then
	echo 'Warning: fzf is required for the fzf-tab plugin but was not found.'
else
	setopt globdots
...
TRAPUSR1() {
  local colors_file="$ZDOTDIR/.p10k.colors.zsh"
  [[ -e $colors_file ]] || return
  source "$colors_file"
